...
def process_frame(frame, frame_count, index):
    try:
        ...
        smoothed_image = cv2.cuda.bilateralFilter(gray, 35, 120, 120)
        edged_image = cv2.cuda.Canny(smoothed_image, 140, 190)
        ...
        keypoints = cv2.findContours(
            eroded_image.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
...
def start_processing():
    ...
    Thread(target=process_video, args=(
        video_path, output_path, label_status)).start()
